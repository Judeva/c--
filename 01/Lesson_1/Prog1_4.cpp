// Програма Prog1_4.cpp
// Демонстрация на подаване на аргументи към конструктор.
// Напрвена са корекции в qput() и qget() след открит бъг на 16.02.2021 г. 
#include <iostream> 
using namespace std;
...
//	sloc++; 
	q[sloc++] = i; // така пишем, започвайки от q[0]
}
...
//	cout << "num = " << num << "  rloc = " << rloc  <<  "sloc = " << sloc  << endl;
	if (rloc == sloc) {
		cout << "Опашка " << num << " е празна.\n"; 
		return 0;
	}
//	rloc++; 
	return q[rloc++];// така четем, започвайки от q[0]
}
